...
public class CompilerBaseClass {
  ...
  boolean INICIA () {
    while ( true ) {
      ...
      if ( num_item >= itensLex.size() ) {
        ...
        if (Configuracao.listaAnaSim)
          System.out.println("\n[CompilerBaseClass!INICIA] cadeia reconhecida e gerado código com sucesso!");
        return true;
        }
      } // while ( true )
    }
  ...
  boolean Tpar () {
    ...
    if ( tipo(num_item) == Elemento.OUTROS && item(num_item).equals( "(" ) ) {
      ...
      avanca_item();
      if ( !EA() ) { //N
	 // Erro: esperava um operando após  \'(\' (encontrado \'" + item(num_item) + "\')"
  ...
  */
  boolean EXPLOG () {
    if (Configuracao.listaAnaSim)
      System.out.println("[CompilerBaseClass!EXPLOG] Início: EXPLOG := ELSS | ELS ou ELS");
    ...
    try {
      if (ELS()) {
        ...
        System.out.println("[CompilerBaseClass!EXPLOG] Erro em expressão lógica: esperava [ELS] ou [ELS || ELS]");// ERRO
        return false;
        }
  ...
  */
  boolean ELS () {
    if (Configuracao.listaAnaSim)
      System.out.println("[CompilerBaseClass!ELS] Início: ELS := ELSS (e ELSS)* | ! ELS");
      ...
      if (ELSS()) {
        ...
        if (algumE) {
          ...
          for (int i=0; i<vetBlocosEs.size(); i++) {
              ...
              if (Configuracao.listaAnaSim)
                 System.out.println("\n[CompilerBaseClass!EXPLOG] substituido \"9ee\" por \"9"+pM+"\" na pos. memória "+posInstrE);
              }
          }
  ...
  boolean EA () {
    if (Configuracao.listaAnaSim)
      System.out.println("[CompilerBaseClass!EA] inicio:  [+|-] T* [+|-|ou] (ou é '||')");
    ...
    try {
      ...
      if (tipo(num_item)==Elemento.OUTROS &&
         ...
         // System.out.println("[CompilerBaseClass!EA] unário: "+item(num_item)+" "+tipo(num_item));
         ...
         if ( T() ) {
            ...
            if (primeiro_T) {
               if (menos_unario) {
                  ...
                  programa.adicionaComando("1$"+endPilhaExec+topo);     // $(EE)   <- cAC           ; usa pos. de mem. (mas não registra)
            ...
            if (op!='X') { // já ocorreu algum operador antes do último T reconhecido => realize operação
               if (op == '+') {
                  // Atencao ao uso de 'topo' gerenciado em 'T()', 'endPilhaExec' gerenciado em 'EA(),ELSS()': usados em 'EA()' e 'T()'
                  endPilhaExec--;                                  //                          ; libera últ. posição de memória usada
                  programa.adicionaComando("2$"+endPilhaExec+topo); // AC      <- cAC + c$(EE-1); pega últ. valor na pilha de mem.
                  }
               else
               if (op == '-') {
                  // Atencao ao uso de 'topo' gerenciado em 'T()', 'endPilhaExec' gerenciado em 'EA(),ELSS()': usados em 'EA()' e 'T()'
           ...
           if ( num_item >= itensLex.size() - 1 ) {
             // Erro: final de cadeia após um operador unário (\'+\' ou \'-\'), esperava operando
             System.out.println("[CompilerBaseClass!EA] Erro, não reconhecido T "+item(num_item)+" "+tipo(num_item));
  ...
  boolean T () {
    if (Configuracao.listaAnaSim)
      System.out.println("[CompilerBaseClass!T] início ");
    String item_op = null;
    //String codigoParcial = null;
    //ListaLigada posicoes = new ListaLigada();
    char op = 'X';
    ...
    try {
      ...
      while ( true ) {
        ...
        if ( FATOR() ) {
          ...
          //System.out.println("[CompilerBaseClass!T] após FATOR(): temFator=" + temFator +"  <"+item(num_item)+"> num_item="+num_item);
          ...
          if ( tipo(num_item) == Elemento.OUTROS &&
           ...
           System.out.println("[CompilerBaseClass!T] erro, não encontei um FATOR -> num_item="+num_item);
      ...
      //  Erro: final de cadeia após fator de Expressão Aritmética
  ...
  boolean FATOR () {
    ...
    if ( tipo(num_item) == Elemento.VARIAVEL ) {
      ...
      programa.aloca(strItemVar); // aloca espaço para a var. ou pega um end. EE já existente
    ...
    if ( item(num_item).equals("(") ) {  // o '(' não está com valor "OUTROS"!!
      ...
      if ( ELSS() ) {
        ...
        if ( item(num_item).equals(")") ) {
           ...
           System.out.println("[CompilerBaseClass!FATOR] --- NÃO reconhecido o fecha de \"(ELSS)\" "+item(num_item)+" "+tipo(num_item));
           return false;
           }
        }
      else {
        System.out.println("[CompilerBaseClass!FATOR] NÃO ELSS após um \"( "+item(num_item)+" "+tipo(num_item));
        return false;
        }
      } // if ( tipo(num_item) == Elemento.OUTROS && item(num_item).equals("(") )
    else
    if ( tipo(num_item) == Elemento.OUTROS && item(num_item).equals("!") ) {
      ...
      if ( FATOR() ) {
        ...
        System.out.println("[CompilerBaseClass!FATOR] --- NÃO reconhecido FATOR de \"!FATOR\" "+item(num_item-1)+" "+tipo(num_item-1));
        return false;
        }
      } // if ( tipo(num_item) == Elemento.OUTROS && item(num_item).equals("!") )
    else {
      //System.out.println("[CompilerBaseClass!FATOR]---NÃOreconhecidoNUM|ID|(ELSS)|!FATOR:item="+item(num_item)+"tipo="+tipo(num_item));
      System.out.println("[CompilerBaseClass!FATOR] --- NÃO reconhecido NUM|ID|(ELSS): item="+item(num_item)+" tipo="+tipo(num_item));
    ...
    if ( EA() ) { // acabou de calcular primeira expressão aritmética
      ...
      if (temOpRel) {
        ...
        if ( EA() ) { // E OpRel E
           ...
           System.out.println("[CompilerBaseClass!ELSS] --- NÃO reconhecido EA final em ELSS!! "+item(num_item)+" "+tipo(num_item));
           return false;
           }
        } // if (temOpRel)
      else {
      ...
      System.out.println("[CompilerBaseClass!ELSS] NÃO reconhecido  ELSS := EA [op EA] "+item(num_item)+" "+tipo(num_item));
      return false;
      }
    } // boolean ELSS()
    ...
    try {
      ...
      if ( tipo(num_item) == Elemento.COMANDOS && item(num_item).equals(Bundle.msg("cmdIf")) ) { // "if"
         ...
         if ( !IF() ) { // "Nonterminal" item recognition failed: it not a "(EL)", i.e., it is not a "logical expression" for "if (EL)"
           pilhaDeInformacoes = pilhaDeInformacoes.concat( "\n" + Bundle.msg("msgCompErrELexpect")); // Erro: Era esperado Expressão lógica.
           return false;
           }
         else {
           programa.esvaziaAC();
       ...
       // identifica a parte da leitura de variáveis
         ...
         if ( !IO_LE() ) { // "Nonterminal" item recognition failed: it not a "variable" for "read(...)"
            pilhaDeInformacoes = pilhaDeInformacoes.concat("\n" + Bundle.msg("msgCompErrParamVar")); // Erro: Parâmetros incorretos, era esperada uma variável
            return false;
            }
       ...
       // se é operando
         ...
         if ( tipo(num_item) == Elemento.OUTROS && item(num_item).equals( "=" )) {
           ...
           if ( !EA() ) { //"Nonterminal" item recognition failed: it not a "arithmetic expression" for <atribution> "="
              pilhaDeInformacoes = pilhaDeInformacoes.concat("\n" + Bundle.msg("msgCompErrExpectEA")); // Erro: Era esperada uma expressão aritmetica (para variavel a esquerda)
              return false;
              }
           else {
        ...
        pilhaDeInformacoes = pilhaDeInformacoes.concat("\n" + Bundle.msg("msgCompErrExpectVar")); // Erro: era esperada uma variável       
  ...
  boolean IF () {
    ...
    if (Configuracao.listaAnaSim)
      System.out.println("[CompilerBaseClass!IF] início - "+item(num_item)+": "+tipo(num_item));
    ...
    try {
      if ( tipo(num_item) == Elemento.OUTROS && item(num_item).equals("(") ) {
        ...
        if ( EXPLOG() ) { // traduz a expressão lógica: AC = 0 <=> EL = false
           if ( tipo(num_item) != Elemento.OUTROS || !item(num_item).equals(")") ) {
	     // Erro: esperava um \')\' após uma Expressão Lógica
             pilhaDeInformacoes = pilhaDeInformacoes.concat("\n" + Bundle.msg("msgCompErrExpectCloseParEL"));
             return false;
             }
           else {
    ...
    try {
      ...
      if ( tipo(num_item) == Elemento.OUTROS && item(num_item).equals( "(" ) ) {
        ...
        while ( true ) {
          ...
          else { // Erro: era esperada uma variável
            pilhaDeInformacoes = pilhaDeInformacoes.concat("\n" + Bundle.msg("msgCompErrExpectVar"));
            //System.out.println("[icg.compilador.CompilerBaseClass!IO_LE] erro: \"Erro: era esperado variável.\" em: "+item(num_item));
            return false;
            }
          } // while ( true )
      ...
      if ( tipo(num_item) == Elemento.OUTROS && item(num_item).equals( "(" ) ) {
        ...
        while ( true ) {
          ...
          if ( tipo(num_item) == Elemento.VARIAVEL ) { // || tipo(num_item) == Elemento.NUMERO ) {
            ...
            int numAux = num_item-1;
            String [] strMsgs = { item(numAux), ""+numAux };
            String msgErr = Bundle.msg("msgCompErrExpectVar") + "! " + // Erro: era esperada uma variável
                            Bundle.msgComVar("msgCompErrFound","OBJ",strMsgs); // Encontrei um \"<item(n)>"\ (no item <n>)
            pilhaDeInformacoes = pilhaDeInformacoes.concat("\n" + msgErr);
